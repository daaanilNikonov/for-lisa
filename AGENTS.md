...
- There are **no tests, no linter, and no CI** configured. "End-to-end verification" = run the two build
  scripts and confirm the `.pptx` files open (e.g. reopen with `python-pptx` and check the slide count printed
  by each script matches `len(prs.slides)`).
