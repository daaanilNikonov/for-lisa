...
## Файл для загрузки на домен

Готовый архив:

- [`dist/karta-dnya-domen.zip`](./dist/karta-dnya-domen.zip) (короткое имя)
- или `dist/karta-dnya-forus-domen-YYYYMMDD.zip`

Инструкция внутри архива: **`ЗАГРУЗКА-НА-ДОМЕН.txt`**.

Пересобрать архив:

```bash
cd karta-dnya
./scripts/pack-domain.sh
```

Нужен Node.js 18+ на сервере (или хостинг с поддержкой Node).
